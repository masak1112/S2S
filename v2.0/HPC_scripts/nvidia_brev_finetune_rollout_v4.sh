...
python -m torch.distributed.launch --master_port=29504 --nproc_per_node=$NUM_TASKS_PER_NODE \
    ...
    --run_num=finetune_rollout_crps_multipsteps \
    --epochs=10 \
    --run_iter=1 \
    --max_rollout_steps=20 \
    --rollout_step_interval=500 \
    --initial_rollout_steps=3 \
    --finetune_ckpt=results/S2S/rollout_finetune_finetune_rollout_crps/training_checkpoints/rollout_ckpt_1000.tar
