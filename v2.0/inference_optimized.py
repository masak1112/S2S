            ...
            for i, data in enumerate(self.valid_data_loader, 0):
                ...
                for ens_id in list(range(2)): 
                    ...
                    upper_t = self.valid_dataset.upper_air_inv_transform(
                        _val_output_upper_air_gpu.view(B * T, *_val_output_upper_air_gpu.shape[2:]))
                    diag_t = self.valid_dataset.diagnostic_inv_transform(
                        _val_output_diagnostic_gpu.view(B * T, *_val_output_diagnostic_gpu.shape[2:]))

                    # Lazy-allocate pinned CPU buffers to avoid repeated cudaHostAlloc
                    if _surf_cpu is None or _surf_cpu.shape != surf_t.shape:
                        _surf_cpu = torch.empty(surf_t.shape, dtype=torch.float32, pin_memory=True)
                    if _upper_cpu is None or _upper_cpu.shape != upper_t.shape:
                        _upper_cpu = torch.empty(upper_t.shape, dtype=torch.float32, pin_memory=True)
                    if _diag_cpu is None or _diag_cpu.shape != diag_t.shape:
                        _diag_cpu = torch.empty(diag_t.shape, dtype=torch.float32, pin_memory=True)

                    # Async D2H into pinned buffers, then one sync for all three
                    _surf_cpu.copy_(surf_t, non_blocking=True)
                    _upper_cpu.copy_(upper_t, non_blocking=True)
                    _diag_cpu.copy_(diag_t, non_blocking=True)
                    torch.cuda.synchronize()

                    # .copy() makes independent numpy arrays so pinned buffers can be reused next iteration
                    val_output_surface = _surf_cpu.numpy().reshape(B, T, *_val_output_surface_gpu.shape[2:]).copy()
